...
using TreapNode = Algorithms.Graphs.DynamicGraph.TreapNode;

[TestFixture]
public class TreapTest
{
    Random _random = new Random(0x12345678);
    ...
    public static List<object> TreapToList(TreapNode treap)
    {
        ...
        for (var cur = treap.First(); cur != null; cur = cur.Next)
            l.Add(cur.Value);
...
//Check priorities, generate inorder list
    ...
    public void TreapRandom()
    {
        ...
        for (var i = 0; i < 100; ++i)
        {
            var h = (int) (_random.NextDouble() * list.Count);
            var n = nodes[h];
            ...
            {
                var pp = (int)(_random.NextDouble() * list.Count);
                var s = _random.NextDouble() < 0.5;
                nodes[pp].SetFlag(s);
    ...
    public void TreapMergeSplitSimple()
    {
        ...
        for (var i = 1; i < 10; ++i)
        {
            ...
            {
                nodes[nodes.Count - 1].SetFlag(true);
            }
        }
        ...
        for (var i = 0; i < 9; ++i)
        {
            var lo = list.GetRange(0, i + 1);
            var hi = list.GetRange(i + 1, list.Count -(i+1));

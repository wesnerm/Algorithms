...
namespace Algorithms.Graphs
{
    ...
    public class EulerianPathTest
    {
        ...
        public void DirectedOddTest()
        {
            ...
            var path = euler.FindPath(0);
            Assert.AreEqual(new[] { 0, 1, 2 }, path);
        ...
        public void DirectedEvenTest()
        {
            ...
            var path = euler.FindPath(0);
            Assert.AreEqual(new[] { 0, 1, 2, 0 }, path);

            path = euler.FindPath(1);
            Assert.AreEqual(new[] { 1, 2, 0, 1 }, path);

            path = euler.FindPath(2);
            Assert.AreEqual(new[] { 2, 0, 1, 2 }, path);
        }
        ...
        public void UndirectedOddTest()
        {
            ...
            path = euler.FindPath(2);
            Assert.AreEqual(new[] { 2, 1, 0 }, path);
        }
        ...
        public void UndirectedEvenTest()
        {
            ...
            path = euler.FindPath(1);
            Assert.AreEqual(new[] { 1, 0, 2, 1 }, path);

            path = euler.FindPath(2);
            Assert.AreEqual(new[] { 2, 1, 0, 2 }, path);
        }
    }
}
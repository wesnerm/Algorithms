...
public class PolynomialOperationsTest
{
    ...
    public void InvertTest()
    {
        ...
        for (int it = 0; it < 100; ++it)
        {
            ...
            {
                poly1[i] = r.Next(-2, MOD);
            }

            var poly2 = Invert(poly1, poly1.Length, MOD);
            var poly3 = Invert(poly2, poly2.Length, MOD);
            Assert.IsTrue(ArrayEqual(Massage(poly1), Massage(poly3)));
        }
    }
    ...
    public void DivTest()
    {
        ...
        for (int it = 0; it < 1000; ++it)
        {
            ...
            long[] z = MultiplyPolynomialsMod(x, y, MOD, 0);
            z = Trim(z);
            ...
            var q = DivPolynomial(z2, x, MOD);
            var rem = ModPolynomial(z2, x, MOD);

            Massage(rem);
            Massage(q);

            Assert.IsTrue(ArrayEqual(y, q));
            Assert.IsTrue(ArrayEqual(rem, poly));
        }
    }

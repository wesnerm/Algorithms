...
public class BernoulliNumbersTests
{
    ...
    {
        var b = BernoulliNaive(50, MOD);
        Assert.AreEqual(1, Fix(b[0], MOD));
        Assert.AreEqual(1, Fix(b[1] * 2, MOD));  // Can be + or -; newer users -
        Assert.AreEqual(1, Fix(b[2] * 6, MOD));
        Assert.AreEqual(0, b[3])    ;
        Assert.AreEqual(-1, Fix(b[4] * 30, MOD));
        Assert.AreEqual(1, Fix(b[6] * 42, MOD));
    }

    [Test]
    public void BernoulliTest()
    {
        var b = BernoulliNaive(51, MOD);
        var b2 = Bernoulli(51, true);
    ...
    {
        var bernoulli = BernoulliNaive(52, MOD);
        var f = FaulhabersTableForFixedN(bernoulli, 40, 50, MOD);
        var test = FaulHabersTable2(bernoulli, 40, 50);

    ...
    IComparable<Number>, IEquatable<Number>
{
    ...
    [FieldOffset(0)] private double _real;
    [FieldOffset(0)] private ulong _num;
    [FieldOffset(8)] private long _den;
        ...
        : this()
    {
        ...
        if (den != 1)
        {
            var g = NumberTheory.Gcd(num, den);
            _num /= (ulong) g;
            _den /= g;
        }
    }
        ...
        : this()
    {
        ...
        if (den != 1)
        {
            ...
            {
                var g = NumberTheory.Gcd(num, (ulong) (den > 0 ? den : unchecked(-den)));
                if (g != 1)
                {
                    _num /= g;
                    _den /= (long) g;
    ...
    public bool IsMultipleOf(ulong n)
    {
        return (_den == 1 || _den == -1) && _num%n == 0;
    }

    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    public bool IsEven
    {
        get { return (_den == 1 || _den == -1) && (_num & 1) == 0; }
    }
    ...
    {
        get { return _den == 0 && double.IsInfinity(_real); }
    }

    public bool IsNaN
    {
        get { return _den == 0 && double.IsNaN(_real); }
    }
    ...
    public Number Fraction()
    {
        unchecked
        {
            ...
            var dn = _den > 0 ? (ulong) _den : (ulong) -_den;
    ...
    {
        get { return _den < 0 || _den == 0 && _real < 0; }
    }

    public bool IsNormal
    {
        get { return _den != 0 || _real < double.PositiveInfinity && _real > double.NegativeInfinity; }
    }
    ...
    {
        return TryParse(s, NumberStyles.Float | NumberStyles.AllowThousands, out number);
    }
    ...
    private static void AddRational(ref Number n1, ref Number n2)
    {
        unchecked
        {
            if (n2._num == 0) return;
            ...
            double n;
            double d = n1._den;
            var g = NumberTheory.Gcd(d, n2._den);
            if (g == 1.0)
            {
    ...
    private void MultiplyRational(ulong n2, ulong d2)
    {
        if (n2 == 0 || _num == 0)
        {
            this = Zero;
        ...
        var gcd2 = NumberTheory.Gcd(n2, _den);
        var n = _num/gcd1*(n2/gcd2);
        var d = _den/gcd2*(d2/gcd1);
    ...
    {
        return (n1._den == 0 ? n1._real : n1._num/(double) n1._den)
    ...
    {
        var cmp = Value.CompareTo(n.Value);
        if (cmp != 0) return cmp;
    ...
    public static Number GCD(Number n1, Number n2)
    {
        unchecked
        {
            ...
            var g = NumberTheory.Gcd((double) n1._den, n2._den);
            var lcm = n1._den/g*n2._den;
            if (lcm <= MaxLong)
            {
    ...
    {
        return (_den == 0 ? _real : _num/(double) _den)
               == (n2._den == 0 ? n2._real : n2._num/(double) n2._den);
    }

    public override bool Equals(object number)
    {
        return number is Number && Equals((Number) number);
    }
    ...
    public bool Match(ulong num, long den)
    {
        return _num == num && _den == den;
    }
    ...
    public bool Match(ulong num)
    {
        return _den == 1 && _num == num;
    }

    public bool Match(double d)
    {
        return _den == 0 && _real.Equals(d);
    }

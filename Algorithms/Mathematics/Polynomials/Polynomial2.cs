...
public static class Polynomial2
{
    ...
    private const int L = 15; // can be 23 for 998244353
    private const int N = 1 << L;
    private static bool fftInitialized = false;

    private const int mod = 998244353;
    private const int MAXN = 3030;
    private static List<int>[] a = new List<int>[MAXN];
    private static int root = GetRoot();

    private static long[] angles = new long[N + 1];
    private static int[] bitrev = new int[N];
    ...
    private static int RevBit(int x, int len)
    {
        return bitrev[x] >> (L - len);
    }

    private static void Fft(long[] a, bool inverse = false)
    {
    ...
    private static long[] Multiply(long[] a, long[] b, int size = 0)
    {
        int n = 1;
    ...
    private static long[] Integrate(long[] a)
    {
        if (a.Length == 0)
            return a;
    ...
    {
        var result = GetRange(a, 0, Math.Max(a.Length, b.Length), true);
    ...
    {
        var len = (size == -1) ? a.Length : Math.Min(size, a.Length);
    ...
    public static long[] GetInverse(long[] a, int prec)
    {
        ...
        var res = new long[] { InverseDirect((int)a[0], mod) };
        int k = 1;
        while (k < prec)
        {
            k *= 2;
            var tmp = Multiply(res, GetRange(a, 0, Math.Min(k, a.Length)));
    ...
    public static List<long> Multipoint(long[] poly, long[] pts)
    {
        ...
        var segment_polys = GetSegmentProducts(pts);
    ...
    private static List<long[]> GetSegmentProducts(long[] pts)
    {
        ...
        fillPolys = (int left, int right) =>
        {
            ...
            var newPoly = Multiply(segmentPolys[i], segmentPolys[j]);
    ...
    private static long[] Build(long[][] result, long[] pts, int left, int right, int v=1)
    { 
        if (right - left == 1)
    ...
    private static long[] Interpolate(long[] xs, long[] ys)
    {
        ...
        var segment_polys = GetSegmentProducts(xs);
        var der = Derivative(segment_polys[segment_polys.Count - 1]);
        var coeffs = Multipoint(der, xs);
        ...
        getAns = () =>
        {
            ...
            {
                segment_polys.RemoveAt(segment_polys.Count - 1);
                var res = new long[] { coeffs[coeffs.Count - 1] };
            ...
            else
            {
                ...
                var p1 = segment_polys[segment_polys.Count - 1];
                var q1 = getAns();
                var p2 = segment_polys[segment_polys.Count - 1];
                var q2 = getAns();
    ...
    private static long[] Logarithm(long[] a, int prec)
    {
        Debug.Assert(a[0] == 1);
    ...
    private static long[] Exponent(long[] a, int prec)
    {
        Debug.Assert(a[0] == 0);
        var res = new long[] { 0 };
        int k = 1;

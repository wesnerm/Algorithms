...
public static class PolynomialOperation
{
    ...
    public static int GFPow(int[] A, int[] c, long K, int MOD)
    {
        ...
        while (k >= a.Length)
        {
            ...
            long[] combine = MultiplyPolynomialsMod(poly, negpoly, MOD, 0);
            ...
            for (int i = a.Length; i < a2.Length; i++)
            {
                ...
                {
                    tmp = (tmp + (MOD - poly[j + 1]) * a2[i - 1 - j]) % MOD;
                }
                a2[i] = (int)tmp;
            }

            int[] newA = new int[a.Length];
            for (int i = 0, j = (int)(k & 1); i < newA.Length && j < a2.Length; i++, j += 2)
            {
                newA[i] = a2[j];
            }
    ...
    public static long[] GetCoefficientsForPower(long[] coefs, int k, long MOD)
    {
        ...
        for (int i = 1 << 29; i > 0; i >>= 1)
        {
            if (2 * i <= k)
            {
                b = ModPolynomial(MultiplyPolynomialsMod(b, b, MOD, 0), m, MOD);
            }
            if ((k & i) != 0)
            {
                b = ModPolynomial(MultiplyPolynomialsMod(b, x, MOD, 0), m, MOD);
            }
        }
    ...
    private static long[] Invert2(long[] f, int n)
    {
        // assert f[0] == 1
        // assert f != r
    ...
    // O(n log n)
    private static long[] Divide2(long[] p, long[] d, long[] R)
    {
    ...
    public static long[] Shift(long[] f, long a)
    {
        ...
        var uv = MultiplyPolynomialsMod(u, v, MOD, n);
        var result = new long[n];
        var ifactnm1 = InverseFact(n - 1);
        for (int i = 0; i < n; i++)
    ...
    public static long[] Compose(long[] f, long[] g)
    {
    ...
    public static long[] Reversion(long[] f, long[] g)
    {
        throw new NotImplementedException();
    }

...
public static class Polynomials
{
    ...
    public static long[] AddPolynomials(long[] a, long[] b, long[] result = null)
    {
        ...
        if (result != a) for (int i = 0; i < a.Length; i++) result[i] = a[i];
    ...
    {
        size = size > 0 ? Math.Min(a.Length + b.Length - 1, size) : a.Length + b.Length - 1;
        long[] result = new long[size];
        for (int i = 0; i < result.Length; i++)
        {
            long r = 0;
            int limit = Math.Min(a.Length, i + 1);
            for (int j = Math.Max(0, i - b.Length + 1); j < limit; j++)
                r += a[j] * b[i - j];
            result[i] = r;
        }
        return result;
    ...
    public static long[] MultiplyPolynomialsModFastest(long[] a, long[] b, int MOD, int size = 0)
    {
        unchecked
        {
            ...
            for (int i = 0; i < result.Length; i++)
            {
                long r = 0;
                int limit = Math.Min(a.Length, i + 1);
                for (int j = Math.Max(0, i - b.Length + 1); j < limit;)
                {
                    for (int limit2 = Math.Min(j + 18, limit); j < limit2; j++)
                        r += a[j] * b[i - j];
    ...
    public unsafe class AdvancePolyMath
    {
        ...
        {
            if (n == 1)
                return new long[] { ModularMath.ModInverse(a[0], mod) };

            int nn = n + 1 >> 1;
            var b0 = Invert(a, nn, mod);
            var b = new long[n];
            Array.Copy(b0, 0, b, 0, nn);
            var c = MultiplyPolynomialsMod(b0, b0, mod);
            var ac = MultiplyPolynomialsMod(a, c, mod);
            for (int i = 0; i < n; i++)
            {
                b[i] = 2 * b[i] - ac[i];
                if (b[i] >= mod) b[i] -= mod;
    ...
    {
        double[] remainder;
        var quotient = DividePolynomialCore(dividend, divisor, true, out remainder);
        return quotient;
    }

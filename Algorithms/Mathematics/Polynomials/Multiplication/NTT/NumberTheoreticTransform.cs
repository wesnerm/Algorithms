...
public class NumberTheoreticTransform2
{
    ...
    static long[] abuffer = new long[1 << 20];
    static long[] bbuffer = new long[1 << 20];
    ...
    public static unsafe long[] Multiply(long[] a, long[] b, int size = 0)
    {
        ...
        int alen = a.Length;
        int blen = b.Length;
        if (size == 0 || productSize < size) size = productSize;
        else
        {

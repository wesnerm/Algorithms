...
public class FftTourist
{
    ...
    public static int[] Multiply(int[] a, int[] b)
    {
        ...
        {
            int j = (sz - i) & (sz - 1);
            var z = (fa[j] * fa[j] - Complex.Conjugate(fa[i] * fa[i])) * r;
        ...
        for (int i = 0; i < (sz >> 1); i++)
        {
            var A0 = (fa[i] + fa[i + (sz >> 1)]) * new Complex(0.5, 0);
            var A1 = (fa[i] - fa[i + (sz >> 1)]) * new Complex(0.5, 0) * roots[(sz >> 1) + i];
            fa[i] = A0 + A1 * new Complex(0, 1);
        }
        Fft(fa, sz >> 1);

        var res = new int[need];
    ...
    public static long[] Square(int[] a)
    {
        ...
        {
            int j = ((sz >> 1) - i) & ((sz >> 1) - 1);
            var fe = (fa[i] + Complex.Conjugate(fa[j])) * new Complex(0.5, 0);
            var fo = (fa[i] - Complex.Conjugate(fa[j])) * new Complex(0, -0.5);
            var aux = fe * fe + fo * fo * roots[(sz >> 1) + i] * roots[(sz >> 1) + i];
            var tmp = fe * fo;
    ...
    public static int[] MultiplyMod(int[] a, int[] b, int m, bool eq = false)
    {
        ...
        {
            int j = (sz - i) & (sz - 1);
            var a1 = (fa[i] + Complex.Conjugate(fa[j]));
            var a2 = (fa[i] - Complex.Conjugate(fa[j])) * r2;
            var b1 = (fb[i] + Complex.Conjugate(fb[j])) * r3;
            var b2 = (fb[i] - Complex.Conjugate(fb[j])) * r4;
            if (i != j)
            {
                var c1 = (fa[j] + Complex.Conjugate(fa[i]));
                var c2 = (fa[j] - Complex.Conjugate(fa[i])) * r2;
                var d1 = (fb[j] + Complex.Conjugate(fb[i])) * r3;
                var d2 = (fb[j] - Complex.Conjugate(fb[i])) * r4;
                fa[i] = c1 * d1 + c2 * d2 * r5;
                fb[i] = c1 * d2 + c2 * d1;
            }
            fa[j] = a1 * b1 + a2 * b2 * r5;

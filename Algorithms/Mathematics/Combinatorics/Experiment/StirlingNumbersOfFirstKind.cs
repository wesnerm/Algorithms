...
public class StirlingNumbersOfFirstKind
{
    ...
    {
        if (start == end)
            return new int[] { this.signed ? -start : start, 1 };

        int mid = start + end >> 1;
        var poly1 = ComputeNaive(start, mid);
        var poly2 = ComputeNaive(mid + 1, end);
    ...
    int[] Compute(int n)
    {
        ...
        var stirling = Compute(n >> 1);

        var aux = new int[((n + 1) >> 1) + 1];
        var shift = ShiftStirling(stirling, aux, n >> 1);

...
public static partial class Permutations
{
    ...
    public static long[] CombinationsWithReplacementAtMostK2(int n, int k, int kmax)
    {
        ...
        for (int j = 1; j <= n; j++)
        {
            ...
            {
                int prev = i - kmax - 1;
                var tmp = dp2[i] - (prev < 0 ? 0 : dp2[prev]);
                var result = tmp + dp[i - 1];
                dp[i] = result % MOD;
            }
    ...
    {
        var poly = new long[n + 1];

        for (int i = 0; i <= n; i++)
            poly[i] = 1;

        return Polynomials.PolyPow(poly, m, multiply);
    }
    ...
    {
        return Combinations(nEnd + 1, k + 1) - Combinations(nStart, k + 1);
    }

    // Sum{k=0}^r \binom(m,k) \binom(n,r-k)
    public static long Vandermonde(int n, int m, int r)
    {
        return Combinations(n + m, r);
    }

    // Sum{k=0}^n \binom(m,k) \binom(n,k)
    public static long Vandermonde(int n, int m)
    {
        return Combinations(n + m, n); // = Combinations(n + m, m)
    }
    ...
    {
        var A = new long[odd + 1];
        var B = new long[even + 1];
    ...
    public static long SumChooseOddEvenNeutral(long n, int odd, int even, int either = 0)
    {
        ...
        {
            var x = odd + even + either - 2 * i;
            var num = MOD + 1 - ModPow(x, n + 1);
            var den = MOD + 1 - x;
            var r = Div(num % MOD, den % MOD);

...
public static class MatrixOperations
{
    ...
    {
        int n = a.GetLength(0);
        double[,] tmp = new T[n, n];
        double[,] result = Diagonal(n);
        double[,] b = (T[,])a.Clone();
    ...
    public static bool AreClose(T[,] a, T[,] b)
    {
        ...
        for (int i = 0; i < n; i++)
            for (int j = 0; j < m; j++)
                if (!AreClose(a[i, j], b[i, j])) return false;
        return true;
    }
    ...
    public static T[,] MapPoints(T[][] srcPoints, T[][] destPoints)
    {
        ...
        double[,] matrix = new T[n, n];
        Array.Copy(srcPoints, matrix, srcPoints.Length);
        for (int i = 0; i < n; i++)
            matrix[i, n - 1] = 1;

        double[,] matrix2 = new T[n, n - 1];
        Array.Copy(destPoints, matrix2, destPoints.Length);
        for (int i = 0; i < n; i++)
            matrix2[i, n - 1] = 1;

        var inverse = EquationSolving.Inverse(matrix);

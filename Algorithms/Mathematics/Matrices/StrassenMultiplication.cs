namespace Algorithms.Mathematics.Matrices
{
    public class StrassenMultiplication
    {
        ...
        private static long[][] MatrixMultiply(long[][] a, long[][] b, int n, int l, int p)
        {
            long[][] c = new long[n][];
            for (int i = 0; i < n; i++)
                c[i] = new long[p];
        ...
        private static long[][] MatrixMultiply2(long[][] a, long[][] b, int n, int l, int p)
        {
            // Not actually faster
            long[][] c = new long[n][];
            for (int i = 0; i < n; i++)
        ...
        public static long[,] StrassenMod(long[,] a, long[,] b, int n, int m, int q)
        {
            ...
            for (int i = 0; i < adjN; i++)
            {
                for (int j = 0; j < adjM; j++)
                {
                    var t = p4[i, j] + p3[i, j] - p1[i, j] + p5[i, j];
                    while (t >= MOD) t -= MOD;
                    c[i, j] = t;

...
{
    public unsafe static int[] GaussianElimination(int[][] A, int[] b, int mod)
    {
        int n = b.Length;
        for (int p = 0; p < n; p++)
        {
            ...
            var ipivot = InverseDirect(A[p][p], mod);
            for (int i = p + 1; i < n; i++)
            {
                fixed (int* row = A[i])
                {
    ...
    public unsafe static int[][] Inverse(int[][] A, int mod)
    {
        int n = A.Length;
        ...
        for (int i = n - 1; i >= 0; i--)
        {
            fixed (int* row = A[i])
            {
                var invrowi = InverseDirect(row[i], mod);
                for (int k = 0; k < n; k++)
                {
    ...
    {
        var lu = LUDecompose(matrix, MOD);
        return LUSolve(lu, b, MOD);
    }

     public static long Div(long left, long divisor, long mod) =>
            left * InverseDirect((int)divisor, (int) mod) % mod;
    ...
    public unsafe static int Determinant(int[][] A, int mod)
    {
        int n = A.Length;

...
public class SquareFreeNumbers2
{
    ...
    {
        var limit = (int)MathUtil.Sqrt(max) + 1;

        var factors = Factorization.PrimeFactorsUpTo(limit + 4);
        mobius = Factorization.MobiusTable(limit, factors);
    }
    
    public long GetSquareFreeNumbers(long n)
    {
        var limit = MathUtil.Sqrt(n)+1;
        long result = n;

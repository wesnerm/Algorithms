...
public class Euler
{
    ...
    public void Cut()
    {
        var right = Split(End, left: false);
        var left = Split(this, left: true);
        Join(left, right);
    }

    public void Link(Euler parent)
    {
        var right = Split(parent, false);
        Join(parent, this);
    ...
    {
        Split(root.End, left: false);
        Split(root, left: true);
    }
    ...
    {
        return down1 + (up1 + down2 > 0 ? 0 : down2 + up1);
    }

    public static int UpSummary(int down1, int up1, int down2, int up2)
    {
        return up2 + (up1 + down2 > 0 ? up1 + down2 : 0);
    }
    ...
    private static Euler RecursiveParent(Euler node, int down, int up)
    {
        Debug.Assert(down == 0);
    ...
    public static Euler Lca(Euler u, Euler v)
    {
        ...
        var dftW = RangeLcaSummaryP(rangeRoot);
        Join(predu, u.End);
        Join(u.End, succv);
        var w = NodeAssociatedWith(dftW);
        return TreeParent(w);
    }

    public static Euler TreeRoot(Euler v)
    {
        var pred = Split(v.End, true);
        var rangeRoot = SplayRoot(pred);
        var dftW = RangeLcaSummaryP(rangeRoot);
        Join(pred, v.End);
        var w = NodeAssociatedWith(dftW);
        return w;
    ...
    private static int RangeLcaSummaryP(Euler v)
    {
        throw new NotImplementedException();
    }

    private static Euler NodeAssociatedWith(int p)
    {
        throw new NotImplementedException();
    }

...
public partial class TreeGraphImproved
{
    public struct Node { public int Parent, Index, Size, Head; }
    ...
    #region Construction
    public TreeGraphImproved(List<int>[] g, int root = 0) => Rebuild(g, root);
    ...
    public void Rebuild(List<int>[] g, int root)
    {
        ...
        {
            int n = Math.Max(g.Length, (Trace?.Length ?? 0) * 2);
            node = new Node[n];
            Trace = new int[n];
        }
        Graph = g;
    ...
    public List<Segment> Query(int x, int y, bool edges = false)
    {
        ...
        var lcaIndex = Min(Index(x), Index(y));
        var nodeIndex = Max(Index(x), Index(y));
    ...
    #region MOS
    public void AddTask(List<Task> tasks, int x, int y, Action action) => tasks.Add(new Task { Start = x, End = y, Action = action });

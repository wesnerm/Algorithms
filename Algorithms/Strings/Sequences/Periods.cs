...
public static class Periods
{
    ...
    public static void FindMaximumSpanningPeriodFromRight(int[] seq, out int period, out int maxspan)
    {
        ...
        for (int i = 1; i <= mid; i++)
        {
            ...
            for (; t <= mid; t *= 2)
            {
                ...
                var pre1 = presum[substr1 + t] - presum[substr1];
                var pre2 = presum[substr2 + t] - presum[substr2];
                var good = pre1 == pre2;
            ...
            for (; t >= i; t >>= 1)
            {
                ...
                var pre1 = presum[substr1 + t - 1] - presum[substr1 - 1];
                var pre2 = presum[substr2 + t - 1] - presum[substr2 - 1];
                var good = pre1 == pre2;
                for (int jj = 0; jj < t && good; jj++)
                    good &= seq[substr1 + jj] == seq[substr2 + jj];
                if (good) span += t;
    ...
    public static long ExtrapolatePeriodicSequence(IList<int> list, int k)
    {
        ...
        FindMaximumSpanningPeriodFromRight(diff, 
            out var period, out var span);

        var add = (k - list.Count) / period;
        var rem = (k - list.Count) % period;

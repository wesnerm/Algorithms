...
{
    readonly List<Interval> intervals = new List<Interval>();
    readonly int size;

    public IntervalCounterOffline(int size)
    {
        this.size = size;
    }
    ...
    public void FindSupersets()
    {
        ...
        var leftList = Sorted(intervals);
        int total = 0;
        foreach (var q in leftList)
        {
            if (q.query)
            {
    ...
    class Interval
    {
        ...
        public Interval(int left, int right, Action<int> action = null) : this(left, right, 0)
        {
            this.action = action;
        }

...
namespace Algorithms.Strings.Tries.Alt
{
    public class XorTrieTop
    {
        public XorTrie root;
        XorTrie[] branches;
        public List<XorTrie> list = new List<XorTrie>();
        ...
        void PushDown(XorTrie trie)
        {
            ...
            string s = info;
            int i = s.Length - trie.Length;
            var next = (i < 0 || s[i] == '0') ? trie.left : trie.right;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public XorTrie Insert(string si) => root.Insert(si, 1);
    ...
    public class XorTrie
    {
        ...
        static void FillIds(XorTrie trie, XorTrie[] trace)
        {
            ...
            {
                trace[trie.Id] = trie;
                if (trie.right == null) trie = trie.left;
                else
                {
        ...
        public static XorTrie[] CreateQuickArray(XorTrie root)
        {
            ...
            result[n] = root;
            for (var trie = root; trie.Length > 0; trie = trie.left)

...
public class MergeSortTree
{
    ...
    int Query(int node, int start, int end, int i, int j, int value)
    {
        ...
        if (start >= i && end <= j)
        {
            var answer = Bound(tree[node], value, true);
            return answer;
        }

        int l = 2 * node, r = l + 1, mid = (start + end) >> 1;
        var answer1 = Query(l, start, mid, i, j, value);
        var answer2 = Query(r, mid + 1, end, i, j, value);
        return answer1 + answer2;
    }

    public int Query(int i, int j, int value)
    {
    ...
    int[] QueryList(int node, int start, int end, int i, int j, int size)
    {
        ...
        int l = 2 * node, r = l + 1, mid = (start + end) >> 1;
        var answer1 = QueryList(l, start, mid, i, j, size);
        var answer2 = QueryList(r, mid + 1, end, i, j, size);
        return Merge(answer1, answer2);
    }

    public int[] QueryList(int i, int j, int size)
    {
        return QueryList(1, 0, n - 1, i, j, size);
    }

...
public class SqrtRangeQueryRU
{
    ...
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get { return array[index] + lazy[index >> shift]; }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    {
        var startBorder = Math.Min(end + 1, (start + mask) & ~mask);
        var endBorder = Math.Max(startBorder, end + 1 & ~mask);
    ...
    private static int DecompositionSqrt(int n)
    {
        int shift = 1;

...
public struct PersistentFenwickTree
{
    ...
    {
        var p1 = buckets[pos].Position - 1;
        var e = buckets[pos].Entries;
    ...
    {
        var p = buckets[pos].Position;
        var e = buckets[pos].Entries;
        var lastTime = e[p - 1].Time;
        var ind = time >= lastTime
            ? p + (lastTime == time ? -1 : 0)
            : Bound(e, time, 0, p - 1);
        ...
        if (e.Length == p)
        {
            var newSize = Math.Max(4, e.Length * 2);

...
public struct MatrixFullUpdate
{
    ...
    private void Add(long[,] t, int row, int col, long delta)
    {
        if (col < cols)
    ...
    private long Sum(int row, int col)
    {
        return Sum(t0, row, col) + Sum(t1, row, col) * row * col;
    }

    private static long Sum(long[,] t, int row, int col)
    {
        long sum = 0;
        if (col >= 0)

...
public struct FlipType
{
    ...
    public FlipType Add(FlipType value) => new FlipType { Value = Value + value.Value, FlipValue = FlipValue + value.FlipValue };

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public FlipType Subtract(FlipType value) => new FlipType { Value = Value - value.Value, FlipValue = FlipValue - value.FlipValue };

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public FlipType Scale(int value) => new FlipType { Value = Value * value, FlipValue = FlipValue * value };
...
public class FlipSegmentTree
{
    ...
    public FlipType Sum;
    public FlipType LazyValue = new FlipType();
    ...
    private FlipSegmentTree(FlipType[] array, int start, int length)
    {
        Length = length;
        if (length >= 2)
        {
    ...
    static FlipType Combine(FlipType left, FlipType right)
        => left.Add(right);
    ...
    public FlipType GetSum(int start, int count)
    {
        ...
        LazyPropagate();
        var left = Left.GetSum(start, count);
        var right = Right.GetSum(start - Left.Length, count);
        return Combine(left, right);
    }

    public void CoverInclusive(int start, int end, FlipType v) => Cover(start, end - start + 1, v);
    ...
    public void FlipInclusive(int start, int end) => Flip(start, end - start + 1);

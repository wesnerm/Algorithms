...
public static class Geometry
{
    ...
    public static List<Point2D> ConvexHull(IList<Point2D> points)
    {
        ...
        pts.Sort(); // Sort by X
        var hull = new List<Point2D>(Math.Min(pts.Count, 4));
    ...
    private static void AddToHull(List<Point2D> hull, Point2D p, int limit)
    {
    ...
    public static int FindTangentConvex(Point2D[] poly, Point2D p, int sign)
    {
        ...
        while (left < right)
        {
            ...
            var cmp = p.Cross(poly[mid], poly[mid + 1]) * sign;
            if (cmp == 0)
    ...
    public static double MaxDistance2(IList<Point2D> poly)
    {
        ...
        for (int i = 0, j = n < 2 ? 0 : 1; i < j; ++i)
            ...
            {
                int k = j + 1 < n ? j + 1 : 0;
                result = Math.Max(result, poly[i].Distance2(poly[j]));
    ...
    /// <param name="p"></param>
    /// <returns></returns>
    public static Point2D RotateCcw90(Point2D p) => new Point2D(-p.Y, p.X);

    public static Point2D RotateCw90(Point2D p) => new Point2D(p.Y, -p.X);

    public static Point2D RotateCcw(Point2D p, double t) => new Point2D(p.X * Cos(t) - p.Y * Sin(t),
            p.X * Sin(t) + p.Y * Cos(t));
    ...
    public static bool LinesCollinear(Point2D a, Point2D b, Point2D c, Point2D d) => LinesParallel(a, b, c, d)
    ...
    {
        var dlon = lon * Degrees;
        var dlat = lat * Degrees;
        var rcoslat = r * Cos(dlat);
        x = rcoslat * Cos(dlon);
        y = rcoslat * Sin(dlon);
        z = r * Sin(dlat);
    }
    ...
    public static List<Point2D> Clip(Point2D[] convexClip, Point2D[] polygon)
    {
        ...
        for (int i = 0; i < convexClip.Length; i++)
        {
            ...
            result.Clear();
            var pt0 = convexClip[i];
            var pt1 = convexClip[i + 1 < convexClip.Length ? i + 1 : 0];
            for (int j = 0; j < orig.Count; j++)
            {
                var pt2 = orig[j];
                var pt3 = orig[j + 1 < orig.Count ? j + 1 : 0];
    ...
    {
        var s = (left2 - left1).Cross(right2 - right1);
        return s == 0 ? null : (Point2D?)(left1 + left1.Cross(right1, left2) / s * (left2 - left1));
    }
    public static double Perimeter(IList<Point2D> pts)
    {
        if (pts.Count < 2) return 0;
        var result = pts[0].Distance(pts[pts.Count - 1]);

...
public struct Matrix2D
{
    ...
    public Point2D Offset => new Point2D(OffsetX, OffsetY);
    ...
    public void Rotate(double angle, Point2D pt, ApplyOrder order = ApplyOrder.Append) => Multiply(RotationMatrix(angle).CenterAt(pt), order);

    public void Scale(double sx, double sy, Point2D point = default(Point2D)) => Scale(sx, sy, point, ApplyOrder.Append);

    public void Scale(double sx, double sy, Point2D point, ApplyOrder order) => Multiply(ScaleMatrix(sx, sy).CenterAt(point), order);

    public Point2D TransformPoint(Point2D point) => new Point2D(point.X * E11 + point.Y * E21 + OffsetX,
                            point.X * E12 + point.Y * E22 + OffsetY);

    public Point2D TransformVector(Point2D vector) 
        => new Point2D(vector.X * E11 + vector.Y * E21,
    ...
    public void Translate(double dx, double dy, ApplyOrder order = ApplyOrder.Append) => Multiply(TranslationMatrix(dx, dy), order);
    ...
    public static Matrix2D ScaleMatrix(double sx, double sy) => new Matrix2D
    {
        E11 = sx,
        E22 = sy,
    };

    public static Matrix2D TranslationMatrix(double dx, double dy) => new Matrix2D
    {
        E11 = 1,
        E22 = 1,
        OffsetX = dx,
    ...
    public static Matrix2D ShearMatrix(double shearX, double shearY) => new Matrix2D
    {
        E11 = 1,
        E22 = 1,
        E12 = shearX,
    ...
    public static Point2D operator *(Point2D point, Matrix2D matrix) => new Point2D(
    ...
    [DebuggerStepThrough]
    public Point2D ApplyToPoint(Point2D point2D) => new Point2D(point2D.X * E11 + point2D.Y * E12 + OffsetX,
                          point2D.X * E21 + point2D.Y * E22 + OffsetY);

    [DebuggerStepThrough]
    public Point2D ApplyToVector(Point2D size) => new Point2D(size.X * E11 + size.Y * E12,

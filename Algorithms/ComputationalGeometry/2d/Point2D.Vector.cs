...
public partial struct Point2D
{
    ...
    public Point2D Perp() => new Point2D(-Y, X);
    ...
    public bool IsCloseTo(Point2D vec)
        => Math.Abs(X - vec.X) <= Eps && Math.Abs(Y - vec.Y) <= Eps;

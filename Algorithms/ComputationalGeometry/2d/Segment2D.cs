...
public struct Segment2D
{
    ...
    public Point2D AB => new Point2D(B.X - A.X, B.Y - A.Y);
    ...
    public double DistLine(Point2D point) => A.Equals(B) ? A.Distance(point) : Math.Abs(A.Cross(B, point) / A.Distance2(B));
    ...
    {
        var c = AB.Cross(s.AB);
        return c == 0 ? A + s.A.Cross(s.B, A) * (B - A) / c : (Point2D?)null;
    }
    ...
    public Segment2D? IntersectSegments(Segment2D s)
    {
        ...
        if (a < s.A) a = s.A;
        if (b > s.B) b = s.B;
        return a <= b ? new Segment2D(a, b) : (Segment2D?)null;
    }
}

...
public struct Margin
{
    ...
    public Point2D Size => new Point2D(Left + Right, Top + Bottom);
    ...
    public Point2D Inflate(Point2D v)
    {
        return new Point2D(v.X + Left + Right, v.Y + Top + Bottom);
    }
    ...
    public Point2D Deflate(Point2D size)
    {
        return new Point2D(Math.Max(0, size.X - Width), Math.Max(0, size.Y - Height));
    }

...
public partial struct Point2D : IComparable<Point2D>, IEquatable<Point2D>
{
    ...
    public static Point2D operator +(Point2D p, Point2D p2) => new Point2D(p.X + p2.X, p.Y + p2.Y);

    public static Point2D operator -(Point2D p) => new Point2D(-p.X, -p.Y);

    public static Point2D operator -(Point2D p, Point2D p2) => new Point2D(p.X - p2.X, p.Y - p2.Y);

    public static Point2D operator *(double c, Point2D p) => new Point2D(p.X * c, p.Y * c);

    public static Point2D operator /(Point2D p, double c) => new Point2D(p.X / c, p.Y / c);

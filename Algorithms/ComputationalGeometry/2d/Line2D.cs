...
public struct Line2D
{
    ...
    public Point2D B => new Point2D(A.X + AB.X, A.Y + AB.Y);
    ...
    public Point2D? IntersectClosedSegment(Line2D rhs)
    {
        ...
        return 0 <= ls && ls <= s && 0 <= rs && rs <= s ? A + (ls / s) * AB : (Point2D?)null;
    }

    public Line2D? IntersectSegments(Line2D rhs)
    {
        var s = AB.Cross(rhs.AB);
        var ls = (rhs.A - A).Cross(rhs.AB);
        if (s == 0)
        {
            if (ls != 0) return null;
            ...
            return !(result.AB < default(Point2D)) ? result : (Line2D?) null;
        }
        var rs = (rhs.A - A).Cross(AB);
        if (s < 0) { s = -s; ls = -ls; rs = -rs; }

        bool intersect = 0 <= ls && ls <= s && 0 <= rs && rs <= s;
        return intersect ? new Line2D(A + (ls / s) * AB, new Point2D()) : (Line2D?)null;
    }
}

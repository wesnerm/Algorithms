...
public static class GraphVisualization
{
    ...
    private static void Emit(StringBuilder sb, IEnumerable nodeLabels)
    {
        if (nodeLabels == null) return;
        int i = 0;
        foreach (var obj in nodeLabels)
    ...
    public static string DrawGraph(IList<int>[] graph, string prefix = "", IEnumerable nodeLabels = null)
    {
        ...
        {
            foreach (var v in graph[i])
                if (v >= i) sb.AppendLine($"{prefix}{i} -- {prefix}{v};");
        }
        sb.AppendLine("}");
        return sb.ToString();
    }
    ...
    public static string DrawWeightedDigraph(IList<int>[] graph, IList<int>[] edgeWeights, IEnumerable nodeLabels = null)
    {
        var sb = new StringBuilder();
        sb.AppendLine("digraph {");
        Emit(sb, nodeLabels);
    ...
    public static string PrintGraph(IList<int>[] graph, bool directed = false)
    {
        ...
        {
            foreach (var v in graph[i])
                if (directed || v >= i) sb.AppendLine($"{i} {v}");
        }
        return sb.ToString();
    }

    public static string Join<T>(this IList<T> array, string join = " ")
    {

...
public class Treap
{
    ...
    {
        int n = ks.Length;
        if (n == 0) return new Treap[] { a };
        for (int i = 0; i < n - 1; i++)
            if (ks[i] > ks[i + 1]) 
                throw new ArgumentException();

        Treap[] ns = new Treap[n + 1];
        Treap cur = a;
        for (int i = n - 1; i >= 0; i--)
        {
    ...
    public static Treap[] NodesDfs(Treap a) { return NodesDfs(a, new Treap[a.count], new int[] { 0 }); }

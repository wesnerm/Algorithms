...
public class VanEmdeBoasDense : IEnumerable<int>
{
    ...
    static Node CreateNode(int m)
        => m > 6 ? new VEBNode(m) : (Node)new LeafNode();
    ...
    sealed class VEBNode : Node
    {
        ...
        public override bool Insert(int x)
        {
            ...
            var child = children[i] ?? (children[i] = CreateNode(m2));

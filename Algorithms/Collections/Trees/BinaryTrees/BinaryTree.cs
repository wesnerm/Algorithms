    ...
    where V : IComparable<V>
{
    ...
    public T JoinChildren(T left, T right)
        => Left != left || Right != right ? Join(left, Key, right) : (T)this;
    #endregion
    ...
    private void FillTable(V[] table, ref int index, int start, int end)
    {
        if (IsEmpty) return;
    ...
    public static T Union(T left, T right)
    {
        ...
        if (left.IsEmpty) return right;
        if (right.IsEmpty) return left;
        var split = right.Split(left.Key);
    ...
    public (T left, T right) Split(V key, bool removeKey = true)
    {
        ...
        if (compare < 0)
        {
            var pair = Left.Split(key, removeKey);
            return (pair.left, JoinChildren(pair.right, Right));
        }
        else if (compare > 0)
        {
            var pair = Right.Split(key, removeKey);
    ...
    public V SelectAt(int index, V defaultValue = default(V))
    {
        var node = this;
        while (!node.IsEmpty)
        {
    ...
    public T RemoveRange(int start, int count)
    {
        ...
        int end = start + count;
        int leftCount = Left.Count;
        var leftPart = start < leftCount ? Left.RemoveRange(start, count) : Left;
        int rightStart = Math.Max(start, leftCount + 1);
        var rightPart = end - 1 > leftCount ? Right.RemoveRange(rightStart, end - rightStart) : Right;
    ...
    public T CopyRange(int start, int count)
    {
        ...
        int mid = Left.Count;
        var leftPart = Left.CopyRange(start, count);
        var rightPart = Right.CopyRange(start - (mid + 1), count);
    ...
    public (T left, T right) SplitAt(int pos)
    {
        ...
        if (pos <= leftCount)
        {
            var split = Left.SplitAt(pos);
            return (split.left, JoinChildren(split.right, Right));
        }
        else
        {
            var split = Right.SplitAt(pos - (leftCount+1));
